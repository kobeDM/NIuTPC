...
static const char* mylogger_spec[] = {
    "implementation_id", "NIuTPCLogger",
    "type_name",         "NIuTPCLogger",
...
NIuTPCLogger::NIuTPCLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("niutpclogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("niutpclogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("NIuTPCLogger");
}

NIuTPCLogger::~NIuTPCLogger()
{
}

RTC::ReturnCode_t NIuTPCLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIuTPCLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIuTPCLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIuTPCLogger::daq_dummy()
{
    return 0;
}

int NIuTPCLogger::daq_configure()
{
...
int NIuTPCLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "NIuTPCLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "NIuTPCLogger: Save to file: false\n";
            }
        }
...
int NIuTPCLogger::daq_unconfigure()
{
    std::cerr << "*** NIuTPCLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int NIuTPCLogger::daq_start()
{
    std::cerr << "*** NIuTPCLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: NIuTPCLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** NIuTPCLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int NIuTPCLogger::daq_stop()
{
    std::cerr << "*** NIuTPCLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "NIuTPCLogger::stop: close files \n";
        }
...
int NIuTPCLogger::daq_pause()
{
    std::cerr << "*** NIuTPCLogger::pause" << std::endl;
    return 0;
}

int NIuTPCLogger::daq_resume()
{
    std::cerr << "*** NIuTPCLogger::resume" << std::endl;
    return 0;
}

int NIuTPCLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** NIuTPCLogger::InPort flushed\n";
    }
    return 0;
}

void NIuTPCLogger::toLower(std::basic_string<char>& s)
{
...
int NIuTPCLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        ret = fileUtils->write_data((char *)&m_in_data.data[HEADER_BYTE_SIZE],
                                        event_byte_size);
        if (ret < 0) {
            std::cerr << "### NIuTPCLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "NIuTPCLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void NIuTPCLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<NIuTPCLogger>,
                                 RTC::Delete<NIuTPCLogger>);
    }
};
