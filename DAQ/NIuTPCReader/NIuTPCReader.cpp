...
static const char* niutpcreader_spec[] =
{
    "implementation_id", "NIuTPCReader",
    "type_name",         "NIuTPCReader",
    "description",       "NIuTPCReader component",
    "version",           "1.0",
...
NIuTPCReader::NIuTPCReader(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("niutpcreader_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      m_window_size(4000),
      m_delay(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("niutpcreader_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("NIUTPCREADER");
}

NIuTPCReader::~NIuTPCReader()
{
}

RTC::ReturnCode_t NIuTPCReader::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIuTPCReader::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIuTPCReader::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIuTPCReader::daq_dummy()
{
    return 0;
}

int NIuTPCReader::daq_configure()
{
    fprintfwt(stderr, "NIuTPCReader::configure\n");
    //std::cerr << "*** NIuTPCReader::configure" << std::endl;
...
int NIuTPCReader::parse_params(::NVList* list)
{
...
int NIuTPCReader::daq_unconfigure()
{
    fprintfwt(stderr, "NIuTPCReader::unconfigure\n");
    //std::cerr << "*** NIuTPCReader::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int NIuTPCReader::daq_start()
{
    fprintfwt(stderr, "NIuTPCReader::start\n");
    //std::cerr << "*** NIuTPCReader::start" << std::endl;
...
int NIuTPCReader::daq_stop()
{
    fprintfwt(stderr, "NIuTPCReader::stop\n");
    //std::cerr << "*** NIuTPCReader::stop" << std::endl;
...
int NIuTPCReader::daq_pause()
{
    std::cerr << "*** NIuTPCReader::pause" << std::endl;

    return 0;
}

int NIuTPCReader::daq_resume()
{
    std::cerr << "*** NIuTPCReader::resume" << std::endl;

    return 0;
}

int NIuTPCReader::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int NIuTPCReader::write_OutPort()
{
...
int NIuTPCReader::daq_run()
{
    if (m_debug) {
        std::cerr << "*** NIuTPCReader::run" << std::endl;
    }
...
extern "C"
{
    void NIuTPCReaderInit(RTC::Manager* manager)
    {
        RTC::Properties profile(niutpcreader_spec);
        manager->registerFactory(profile,
                    RTC::Create<NIuTPCReader>,
                    RTC::Delete<NIuTPCReader>);
    }
};
