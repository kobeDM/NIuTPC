...
static const char* niutpcreader0_spec[] =
{
    "implementation_id", "NIuTPCReader0",
    "type_name",         "NIuTPCReader0",
    "description",       "NIuTPCReader0 component",
    "version",           "1.0",
...
NIuTPCReader0::NIuTPCReader0(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("niutpcreader0_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      m_window_size(4000),
      m_delay(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("niutpcreader0_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("NIUTPCREADER0");
}

NIuTPCReader0::~NIuTPCReader0()
{
}

RTC::ReturnCode_t NIuTPCReader0::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIuTPCReader0::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIuTPCReader0::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIuTPCReader0::daq_dummy()
{
    return 0;
}

int NIuTPCReader0::daq_configure()
{
    fprintfwt(stderr, "NIuTPCReader0::configure\n");
    //std::cerr << "*** NIuTPCReader0::configure" << std::endl;
...
int NIuTPCReader0::parse_params(::NVList* list)
{
...
int NIuTPCReader0::daq_unconfigure()
{
    fprintfwt(stderr, "NIuTPCReader0::unconfigure\n");
    //std::cerr << "*** NIuTPCReader0::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int NIuTPCReader0::daq_start()
{
    fprintfwt(stderr, "NIuTPCReader0::start\n");
    //std::cerr << "*** NIuTPCReader0::start" << std::endl;
...
int NIuTPCReader0::daq_stop()
{
    fprintfwt(stderr, "NIuTPCReader0::stop\n");
    //std::cerr << "*** NIuTPCReader0::stop" << std::endl;
...
int NIuTPCReader0::daq_pause()
{
    std::cerr << "*** NIuTPCReader0::pause" << std::endl;

    return 0;
}

int NIuTPCReader0::daq_resume()
{
    std::cerr << "*** NIuTPCReader0::resume" << std::endl;
...
/// from 1board daq
/*
int NIuTPCReader0::get_data_length(unsigned char *buf, int buflen)
{
    unsigned int *length;
    unsigned int rv;
...
int NIuTPCReader0::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int NIuTPCReader0::write_OutPort()
{
...
int NIuTPCReader0::daq_run()
{
    if (m_debug) {
        std::cerr << "*** NIuTPCReader0::run" << std::endl;
    }
...
extern "C"
{
    ...
    {
        RTC::Properties profile(niutpcreader0_spec);
        manager->registerFactory(profile,
                    RTC::Create<NIuTPCReader0>,
                    RTC::Delete<NIuTPCReader0>);
    }
};
