...
static const char* niutpcreader1_spec[] =
{
    "implementation_id", "NIuTPCReader1",
    "type_name",         "NIuTPCReader1",
    "description",       "NIuTPCReader1 component",
    "version",           "1.0",
...
NIuTPCReader1::NIuTPCReader1(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("niutpcreader1_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      m_window_size(4000),
      m_delay(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("niutpcreader1_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("NIUTPCREADER1");
}

NIuTPCReader1::~NIuTPCReader1()
{
}

RTC::ReturnCode_t NIuTPCReader1::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIuTPCReader1::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIuTPCReader1::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIuTPCReader1::daq_dummy()
{
    return 0;
}

int NIuTPCReader1::daq_configure()
{
    fprintfwt(stderr, "NIuTPCReader1::configure\n");
    //std::cerr << "*** NIuTPCReader1::configure" << std::endl;
...
int NIuTPCReader1::parse_params(::NVList* list)
{
...
int NIuTPCReader1::daq_unconfigure()
{
    fprintfwt(stderr, "NIuTPCReader1::unconfigure\n");
    //std::cerr << "*** NIuTPCReader1::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int NIuTPCReader1::daq_start()
{
    fprintfwt(stderr, "NIuTPCReader1::start\n");
    //std::cerr << "*** NIuTPCReader1::start" << std::endl;
...
int NIuTPCReader1::daq_stop()
{
    fprintfwt(stderr, "NIuTPCReader1::stop\n");
    //std::cerr << "*** NIuTPCReader1::stop" << std::endl;
...
int NIuTPCReader1::daq_pause()
{
    std::cerr << "*** NIuTPCReader1::pause" << std::endl;

    return 0;
}

int NIuTPCReader1::daq_resume()
{
    std::cerr << "*** NIuTPCReader1::resume" << std::endl;
...
/// from 1board daq
/*
int NIuTPCReader1::get_data_length(unsigned char *buf, int buflen)
{
    unsigned int *length;
    unsigned int rv;
...
int NIuTPCReader1::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int NIuTPCReader1::write_OutPort()
{
...
int NIuTPCReader1::daq_run()
{
    if (m_debug) {
        std::cerr << "*** NIuTPCReader1::run" << std::endl;
    }
...
extern "C"
{
    void NIuTPCReader1Init(RTC::Manager* manager)
    {
        RTC::Properties profile(niutpcreader1_spec);
        manager->registerFactory(profile,
                    RTC::Create<NIuTPCReader1>,
                    RTC::Delete<NIuTPCReader1>);
    }
};
