...
static const char* niutpcreader3_spec[] =
{
    "implementation_id", "NIuTPCReader3",
    "type_name",         "NIuTPCReader3",
    "description",       "NIuTPCReader3 component",
    "version",           "1.0",
...
NIuTPCReader3::NIuTPCReader3(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("niutpcreader3_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      m_window_size(4000),
      m_delay(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("niutpcreader3_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("NIUTPCREADER3");
}

NIuTPCReader3::~NIuTPCReader3()
{
}

RTC::ReturnCode_t NIuTPCReader3::onInitialize()
{
    if (m_debug) {
        std::cerr << "NIuTPCReader3::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t NIuTPCReader3::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int NIuTPCReader3::daq_dummy()
{
    return 0;
}

int NIuTPCReader3::daq_configure()
{
    fprintfwt(stderr, "NIuTPCReader3::configure\n");
    //std::cerr << "*** NIuTPCReader3::configure" << std::endl;
...
int NIuTPCReader3::parse_params(::NVList* list)
{
...
int NIuTPCReader3::daq_unconfigure()
{
    fprintfwt(stderr, "NIuTPCReader3::unconfigure\n");
    //std::cerr << "*** NIuTPCReader3::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int NIuTPCReader3::daq_start()
{
    fprintfwt(stderr, "NIuTPCReader3::start\n");
    //std::cerr << "*** NIuTPCReader3::start" << std::endl;
...
int NIuTPCReader3::daq_stop()
{
    fprintfwt(stderr, "NIuTPCReader3::stop\n");
    //std::cerr << "*** NIuTPCReader3::stop" << std::endl;
...
int NIuTPCReader3::daq_pause()
{
    std::cerr << "*** NIuTPCReader3::pause" << std::endl;

    return 0;
}

int NIuTPCReader3::daq_resume()
{
    std::cerr << "*** NIuTPCReader3::resume" << std::endl;
...
/// from 1board daq
/*
int NIuTPCReader3::get_data_length(unsigned char *buf, int buflen)
{
    unsigned int *length;
    unsigned int rv;
...
int NIuTPCReader3::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int NIuTPCReader3::write_OutPort()
{
...
int NIuTPCReader3::daq_run()
{
    if (m_debug) {
        std::cerr << "*** NIuTPCReader3::run" << std::endl;
    }
...
extern "C"
{
    void NIuTPCReader3Init(RTC::Manager* manager)
    {
        RTC::Properties profile(niutpcreader3_spec);
        manager->registerFactory(profile,
                    RTC::Create<NIuTPCReader3>,
                    RTC::Delete<NIuTPCReader3>);
    }
};
